<?php
	...
	if (isset($_POST['post'])) {
		...
		// Saves the super global variables to easier variables
		$title = $_POST['title'];
		$img = $_POST['img'];
		...
		// Checks if the required fields aren't empty
		if (isset($title) && isset($img) && isset($cat) && isset($text)) {
			$title = preg_replace('/[^A-Za-z0-9 æ\ø\å\Æ\Ø\Å\-\&]/','',(substr($db->real_escape_string($title), 0, 64)));
			$img = htmlspecialchars(substr($db->real_escape_string($img), 0, 255), ENT_NOQUOTES);
			$caption = htmlspecialchars(substr($db->real_escape_string($caption), 0, 255), ENT_NOQUOTES);
			$cat = substr($db->real_escape_string($cat), 0, 5);
			$text = htmlspecialchars($db->real_escape_string($text), ENT_NOQUOTES);
